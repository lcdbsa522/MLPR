...
parser.add_argument('--kd_alpha', type=float, default=None, help='weight balance for FD')
parser.add_argument('--kd_beta', type=float, default=None, help='weight balance for CE')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model_s, model_t, criterion_list, optimizer_m, optimizer_q, scheduler_m, scheduler_q, epoch, args, writer)

        # evaluate on validation set
        acc1 = validate(val_loader, model_s, model_t, criterion_list, args, writer, epoch)
...
def train(train_loader, model_s, model_t, criterion_list, optimizer_m, optimizer_q, scheduler_m, scheduler_q, epoch, args, writer):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss_total = args.kd_beta * loss_cls + args.kd_gamma * loss_div + args.kd_alpha * loss_fd
